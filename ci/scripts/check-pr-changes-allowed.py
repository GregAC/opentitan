...
def check_changes_against_blocklist(changes: list[str],
        blocklist: list[str]) -> dict[str, list[str]]:
...
def main() -> int:
    arg_parser = argparse.ArgumentParser(
            ...
            description='''Checks a list of changed files supplied as a
                           plain-text list against blocked file patterns. If
                           any match exits with code 1 and the PR should be
                           blocked.''')

    arg_parser.add_argument('changes_file', metavar='changes-file',
