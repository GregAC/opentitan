...
#(
  ...
  always_comb begin
    ...
    if (insn_dec_bignum_i.rf_we) begin
      ...
      end else if (~(insn_dec_shared_i.ld_insn & (state_q != OtbnStateStall))) begin
        // Write both halves for all other instructions, but suppress write for loads when
        // controller isn't in stall state as load data for writeback is only available in the stall
        // state.
  ...
  // For the shift-out variant of MULQACC the bottom half of the MAC result is written to one half
  // of a desintation register specified by the instruction (mac_wr_hw_sel). The bottom half of the
  // MAC result must be placed in the appropriate half of the write data (the RF only accepts write
  // data for the top half in the top half of the write data input). Otherwise (shift-out to bottom
  // half and all other MULQACC instructions) simply pass the MAC result through unchanged as write
  // data.
