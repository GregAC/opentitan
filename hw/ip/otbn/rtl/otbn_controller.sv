...
#(
  ...
  output logic urnd_refresh_req_o,
  input  logic urnd_refresh_busy_i,
  ...
  always_comb begin
    ...
    unique case (state_q)
      OtbnStateHalt: begin
        if (start_i) begin
          state_raw = OtbnStateUrndRefresh;
          urnd_refresh_req_o = 1'b1;
        end
      end
      OtbnStateUrndRefresh: begin
        if (!urnd_refresh_busy_i) begin
          insn_fetch_req_addr_o    = start_addr_i;
  ...
  assign urnd_advance_o = (state_q == OtbnStateRun) | (state_q == OtbnStateStall);
  ...
  `ASSERT(ControllerStateValid, state_q inside {OtbnStateHalt, OtbnStateUrndRefresh, OtbnStateRun,
                                                OtbnStateStall})
  ...
  always_comb begin
    ...
    rf_base_rd_commit_o = 1'b0;

    if (insn_dec_shared_i.ld_insn | insn_dec_shared_i.st_insn) begin
      // For loads and stores read commit happens the same cycle as the request as the read is
      // used to form the request (the address and data if executing a store).
      rf_base_rd_commit_o = (lsu_load_req_o | lsu_store_req_o) & ~err;
    end else begin
      // For all other instructions read commit happens when instruction commits.
  ...
  always_comb begin
    ...
    // Only write if executing instruction wants a bignum rf write and isn't stalled
    if (insn_executing && insn_dec_bignum_i.rf_we && !stall) begin
  ...
  always_comb begin
    ...
    unique case (csr_addr)
      ...
      CsrRndPrefetch: begin
        // Read from RND_PREFETCH results in 0, so no ISPR address set, `csr_rdata` mux logic takes
        // care of producing 0
      end
      CsrUrnd: begin
