 ...
 *
 * This module implements the RND, RND_PREFETCH and URND CSRs. The EDN (entropy
 * distribution network) provides the bits for random numbers. RND gives direct
 * access to EDN bits. URND provides bits from an LFSR that is seeded with bits
 * from the EDN.
 *
 * URND NOT YET IMPLEMENTED
 */
...
// necesarry guarantees required for the various uses of random numbers in OTBN software.         //
////////////////////////////////////////////////////////////////////////////////////////////////////
...
(
  ...
  input  logic            urnd_refresh_req_i,
  // Remains asserted whilst refresh is in progress
  output logic            urnd_refresh_busy_o,
  ...
  localparam LfsrChunksPerWLEN = WLEN / UrndChunkLfsrWidth;
  localparam BytesPerLfsrChunk = UrndChunkLfsrWidth / 8;
  ...
  assign edn_urnd_req_d = (edn_urnd_req_q | urnd_refresh_req_i) & ~edn_urnd_req_complete;

  assign edn_urnd_req_o = edn_urnd_req_q;
  assign urnd_refresh_busy_o = edn_urnd_req_q;
  ...
  for (genvar c = 0; c < LfsrChunksPerWLEN; c++) begin : gen_lfsr_chunks
    localparam logic [UrndChunkLfsrWidth-1:0] LfsrChunkDefaultSeed =
        RndCnstUrndLfsrSeedDefault[c * UrndChunkLfsrWidth +: UrndChunkLfsrWidth];
    ...
    prim_lfsr #(
      ...
      .DefaultSeed ( LfsrChunkDefaultSeed            ),
      .StatePermEn ( 1'b1                            ),
      .StatePerm   ( RndCnstUrndChunkLfsrPermDefault )
    ) u_lfsr_chunk (
      .clk_i     ( clk_i         ),
      .rst_ni    ( rst_ni        ),
